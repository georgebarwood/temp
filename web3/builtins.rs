...
pub fn get_bmap() -> BuiltinMap {
    ...
    let list = [
        ...
        ("SHAONE", DataKind::Binary, CompileFunc::Value(c_shaone)),
...
/// Compile call to SHAONE (SHA1)
fn c_shaone(b: &Block, args: &mut [Expr]) -> CExpPtr<Value> {
    check_types(b, args, &[DataKind::Binary]);
    let input = c_value(b, &mut args[0]);
    lbox!(Shaone { input })
}

/// Compiled call to SHAONE.
struct Shaone {
    input: CExpPtr<Value>,
}
impl CExp<Value> for Shaone {
    fn eval(&self, ee: &mut EvalEnv, d: &[u8]) -> Value {
        let input = self.input.eval(ee, d).bin();
        use sha1::{Sha1, Digest};
        let x = Sha1::digest(&**input);
        let result = LVec::from(&x[..]);
        Value::RcBinary(LRc::new(result))
    }
}
...
impl CExp<i64> for Shutdown {
    fn eval(&self, ee: &mut EvalEnv, d: &[u8]) -> i64 {
        ...
        let mut ext = ee.tr.get_extension();
        if let Some(ext) = ext.downcast_mut::<TransExt>() && let Some(ss) = &ext.ss {
            ss.terminate(code);
        }
        ee.tr.set_extension(ext);
        0
