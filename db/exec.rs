...
fn exec_let(x: &Let, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    let v = x.exp.eval(run, dict, ps);
    run.stack.push(v);
    Ok(())
}

fn exec_set(x: &Set, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    let v = x.exp.eval(run, dict, ps);
...
pub fn execute_fn(f: &SFunc, run: &mut Run, dict: &Dict, ps: &mut PageSet) {
    ...
    execute_gblock(&f.block, run, dict, ps).unwrap();
}

fn execute_gblock(
    slist: &[(usize, GStatement)],
    run: &mut Run,
    dict: &Dict,
    ps: &mut PageSet,
) -> Result<(), E> {
...
fn exec_glet(x: &GLet, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    let v = x.exp.eval(run, dict, ps);
    run.stack.push(v);
    Ok(())
}

fn exec_gset(x: &GSet, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    let v = x.exp.eval(run, dict, ps);
    let ix = run.stack.len() - 1 - x.i;
    run.stack[ix] = v;
    Ok(())
...
fn exec_gwhile(x: &GWhile, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    while x.exp.eval(run, dict, ps).bool() {
...
fn exec_gif(x: &GIf, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    let ok = {
        let v = x.exp.eval(run, dict, ps);
        v.bool()
    };
...
fn exec_ginsert(ins: &GInsert, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    // println!("ins={:?}", ins);
...
fn exec_gupdate(upd: &GUpdate, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    // println!("upd={:?}", upd);
...
fn exec_gdelete(del: &GDelete, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    let t = ps.load_table(del.table.id, &del.table.dt);
...
fn exec_gselect(sel: &GSelect, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    // println!("exec_sel sel={:?}", sel);
...
fn exec_gfor(x: &GFor, run: &mut Run, dict: &Dict, ps: &mut PageSet) -> Result<(), E> {
    // Iterate through table. For each row with valid where condition,
    // push evaluated exps on the stack and execute block.
